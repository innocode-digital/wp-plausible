...
final class Plugin {
	...
	const PROVIDERS = [
		Plugin::PROVIDER_PLAUSIBLE => Plausible\Provider::class,
	];
     ...
     */
    const TEMPLATES = [
        Plugin::TEMPLATE_EMBED,
        Plugin::TEMPLATE_404,
        Plugin::TEMPLATE_SEARCH,
        Plugin::TEMPLATE_FRONT_PAGE,
        Plugin::TEMPLATE_HOME,
        Plugin::TEMPLATE_PRIVACY_POLICY,
        Plugin::TEMPLATE_POST_TYPE_ARCHIVE,
        Plugin::TEMPLATE_TAX,
        Plugin::TEMPLATE_SINGULAR, // Includes 'attachment', 'single' and 'page'.
        Plugin::TEMPLATE_CATEGORY,
        Plugin::TEMPLATE_TAG,
        Plugin::TEMPLATE_AUTHOR,
        Plugin::TEMPLATE_DATE,
        Plugin::TEMPLATE_ARCHIVE,
    ];
	...
	public function __construct( array $allowed_providers = [] ) {
		foreach ( $allowed_providers as $provider ) {
			if ( array_key_exists( $provider, Plugin::PROVIDERS ) ) {
				$class_name = Plugin::PROVIDERS[ $provider ];
    ...
    public function enqueue_scripts(): void {
        ...
        if ( $this->should_track_queried_object() ) {
            $data['queried_object'] = Plugin::get_queried_object();
        }
    ...
    public static function get_queried_object(): array {
        ...
        foreach ( Plugin::TEMPLATES as $template ) {
            if ( call_user_func( "is_$template" ) ) {
                ...
                } elseif ( $template === Plugin::TEMPLATE_POST_TYPE_ARCHIVE ) {
    ...
    public function handle_query(): void {
        ...
        if ( $this->should_track_queried_object() && $query->value() === 'queried_object' ) {
            wp_send_json( Plugin::get_queried_object(), WP_Http::OK );
        }
    }
